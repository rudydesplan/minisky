...
try:
    ...
    # A process we cannot signal is still live and must never be reclaimed.
    raise SystemExit(0)
else:
    raise SystemExit(0)
PY
...
acquire_lock() {
  ...
  for attempt in 1 2 3; do
    ...
    # A creator may briefly exist without complete metadata. Never reclaim an
    # unidentifiable lock because it could belong to a fresh live process.
    ...
    # Rename the exact stale directory out of the lock namespace before
    # removal. Re-read its owner after the atomic rename so a changed/fresh
    # owner is restored rather than deleted.
...
for network in json.load(open(sys.argv[1], encoding="utf-8")):
    for config in (network.get("IPAM") or {}).get("Config") or []:
        ...
        try:
            ...
            pass
for private in (
    # Prefer ranges that are less commonly claimed wholesale by host VPNs or
    # local LAN routes, then fall back to the complete RFC1918 space.
...
JSON
  ...
  rm -rf "${root}"
  trap - EXIT INT TERM
  echo "Phase 16 subnetwork lock and CIDR self-test passed."
}

if [[ "${MINISKY_PHASE16_SUBNETWORK_SELF_TEST:-}" == "1" ]]; then
  run_self_test
  exit 0
fi

if [[ "${MINISKY_PHASE16_SUBNETWORK_INTEGRATION:-}" != "1" ]]; then
  echo "Refusing to start Phase 16 subnetwork/IPAM integration without MINISKY_PHASE16_SUBNETWORK_INTEGRATION=1." >&2
  exit 2
fi
...
if ! docker info >/dev/null 2>&1; then
  echo "Docker daemon is unavailable; Phase 16 subnetwork/IPAM integration requires a real Docker daemon." >&2
  exit 1
fi

shared_lock="${TMPDIR:-/tmp}/minisky-net-integration.lock"
phase_lock="${TMPDIR:-/tmp}/minisky-phase16-subnetwork-integration.lock"
if ! acquire_lock "${shared_lock}" "shared MiniSky Docker integration"; then
  exit 1
fi
if ! acquire_lock "${phase_lock}" "Phase 16 subnetwork/IPAM integration"; then
...
baseline="${work}/subnetwork-baseline.json"
profile="phase16-subnetwork-$$"
project="phase16-subnetwork-$$"
region="us-central1"
network_name="phase16-subnetwork-network"
...
print_current_log() {
  if [[ -n "${current_log}" && -f "${current_log}" ]]; then
    echo "MiniSky Phase 16 subnetwork/IPAM daemon log (${current_log}):" >&2
...
assert_bridge_absent() {
  if docker network inspect "${bridge_id}" >/dev/null 2>&1 ||
    docker network inspect "${bridge_name}" >/dev/null 2>&1; then
    echo "Expected Docker bridge ${bridge_name} (${bridge_id}) to be absent." >&2
    return 1
  fi
}

go build -trimpath -o "${work}/minisky" ./cmd/minisky
go build -trimpath -o "${work}/phase16-subnetwork-smoke" ./sdk-smoke/phase16-subnetwork

export MINISKY_PROJECT_ID="${project}"
export MINISKY_REGION="${region}"
export MINISKY_PHASE16_SUBNETWORK_BASELINE="${baseline}"

start_daemon "first"
# Select only after MiniSky is ready: minisky-net now has a concrete Docker
# allocation and is included in the overlap scan. Export immediately before
# seed so no earlier process can consume a CIDR selected from stale topology.
...
export MINISKY_SUBNETWORK_CIDR="${subnetwork_cidr}"
MINISKY_PHASE16_SUBNETWORK_MODE=seed "${work}/phase16-subnetwork-smoke"
bridge_id="$(inspect_exact_bridge "${bridge_name}" "" "${work}/seed-bridge.json")"
assert_single_canonical_bridge
stop_daemon

# The persisted exact VPC bridge intentionally remains allocated while MiniSky
# is stopped, so later daemon starts cannot claim its selected CIDR.
start_daemon "restarted"
MINISKY_PHASE16_SUBNETWORK_MODE=verify "${work}/phase16-subnetwork-smoke"
inspect_exact_bridge "${bridge_id}" "${bridge_id}" "${work}/restart-bridge.json" >/dev/null
assert_single_canonical_bridge
MINISKY_PHASE16_SUBNETWORK_MODE=cleanup "${work}/phase16-subnetwork-smoke"
assert_bridge_absent
stop_daemon

start_daemon "cleanup-restarted"
MINISKY_PHASE16_SUBNETWORK_MODE=verify-cleanup "${work}/phase16-subnetwork-smoke"
assert_bridge_absent
stop_daemon

duration=$((SECONDS - started_at))
if (( duration > 1200 )); then
  echo "Phase 16 subnetwork/IPAM integration exceeded its 20 minute budget (${duration}s)." >&2
  exit 1
fi
echo "Phase 16 generated-Compute-SDK subnetwork/IPAM Docker restart integration passed in ${duration}s."
