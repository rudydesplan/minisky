...
func TestPhase6CreateContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "memorystore memcached instance id",
			domain:       "memcache.googleapis.com",
			path:         "/v1/projects/test-project/locations/us-central1/instances?instanceId=cache",
			validBody:    `{"nodeCount":1,"nodeConfig":{"cpuCount":1,"memorySizeMb":1024}}`,
			invalidBody:  `{"nodeCount":1,"nodeConfig":{"cpuCount":1,"memorySizeMb":1024}}`,
			invalidQuery: true,
			wantMessage:  "instanceId",
		},
		{
			name:        "memorystore memcached node count",
			domain:      "memcache.googleapis.com",
			path:        "/v1/projects/test-project/locations/us-central1/instances?instanceId=cache",
			validBody:   `{"nodeCount":1,"nodeConfig":{"cpuCount":1,"memorySizeMb":1024}}`,
			invalidBody: `{"nodeConfig":{"cpuCount":1,"memorySizeMb":1024}}`,
			wantMessage: "nodeCount",
		},
		{
			name:        "memorystore memcached node cpu",
			domain:      "memcache.googleapis.com",
			path:        "/v1/projects/test-project/locations/us-central1/instances?instanceId=cache",
			validBody:   `{"nodeCount":1,"nodeConfig":{"cpuCount":1,"memorySizeMb":1024}}`,
			invalidBody: `{"nodeCount":1,"nodeConfig":{"memorySizeMb":1024}}`,
			wantMessage: "nodeConfig.cpuCount",
		},
		{
			name:        "memorystore memcached node memory",
			domain:      "memcache.googleapis.com",
			path:        "/v1/projects/test-project/locations/us-central1/instances?instanceId=cache",
			validBody:   `{"nodeCount":1,"nodeConfig":{"cpuCount":1,"memorySizeMb":1024}}`,
			invalidBody: `{"nodeCount":1,"nodeConfig":{"cpuCount":1}}`,
			wantMessage: "nodeConfig.memorySizeMb",
		},
		{
			name:         "memorystore memcached update mask",
			domain:       "memcache.googleapis.com",
			method:       http.MethodPatch,
			path:         "/v1/projects/test-project/locations/us-central1/instances/cache?updateMask=displayName",
			validBody:    `{"displayName":"updated cache"}`,
			invalidBody:  `{"displayName":"updated cache"}`,
			invalidQuery: true,
			wantMessage:  "updateMask",
		},
		{
