...
| `alloydb.googleapis.com` | experimental | hybrid | experimental | — (not promoted) | Yes | persisted cluster metadata plus exact-owned PostgreSQL primary-instance backend<br>Package tests only: `pkg/shims/alloydb.TestCreateCluster`<br>`pkg/shims/alloydb.TestGetCluster`<br>`pkg/shims/alloydb.TestListClusters`<br>`pkg/shims/alloydb.TestDeleteCluster`<br>`pkg/shims/alloydb.TestPersistAndReload`<br>`pkg/shims/alloydb.TestCreateInstanceIsCanonicalUnsupportedBeforeMutation` | No |
...
| `composer.googleapis.com` | experimental | hybrid | experimental | — (not promoted) | Yes | persisted environment metadata plus exact-owned Airflow backend lifecycle<br>Package tests only: `pkg/shims/composer.TestPersistAndReload` | No |
...
| `documentai.googleapis.com` | experimental | file | experimental | — (not promoted) | Yes | processor metadata and bounded processing<br>Package tests only: `pkg/shims/documentai.TestDocumentAICreatedProcessorSurvivesRestart`<br>`pkg/shims/documentai.TestDocumentAIDeleteOperationSurvivesRestart`<br>`pkg/shims/documentai.TestPersistAndReload` | No |
| `eventarc.googleapis.com` | experimental | file | experimental | — (not promoted) | Yes | trigger metadata and durable Pub/Sub/Storage to Workflow delivery<br>Package tests only: `pkg/shims/eventarc.TestCreateTrigger`<br>`pkg/shims/eventarc.TestDeleteTrigger`<br>`pkg/shims/eventarc.TestPersistAndReload`<br>`pkg/shims/eventarc.TestFailedWorkflowDeliveryReplaysAfterRestart` | No |
| `file.googleapis.com` | experimental | file | experimental | — (not promoted) | Yes | instance metadata and local share data<br>Package tests only: `pkg/shims/filestore.TestLocalShareDataSurvivesMetadataRestart`<br>`pkg/shims/filestore.TestPersistAndReload` | No |
...
| `identityplatform.googleapis.com` | experimental | file | experimental | — (not promoted) | Yes | tenant and IdP configuration metadata<br>Package tests only: `pkg/shims/identityplatform.TestPersistAndReload` | No |
...
| `managedkafka.googleapis.com` | experimental | hybrid | experimental | — (not promoted) | Yes | persisted cluster/topic metadata plus exact-owned Kafka backend lifecycle<br>Package tests only: `pkg/shims/managedkafka.TestPersistAndReload` | No |
...
| `orgpolicy.googleapis.com` | experimental | file | experimental | — (not promoted) | Yes | project policy metadata and local hierarchy evaluation<br>Package tests only: `pkg/shims/orgpolicy.TestPolicyCRUDLifecycle`<br>`pkg/shims/orgpolicy.TestPolicyCreateRestartListDeleteLifecycle` | No |
...
| `storagetransfer.googleapis.com` | experimental | file | experimental | — (not promoted) | Yes | bounded local transfer-job metadata and outcome<br>Package tests only: `pkg/shims/storagetransfer.TestRunTransferJobCopiesObjectsAndPersistsOutcome`<br>`pkg/shims/storagetransfer.TestPersistAndReload`<br>`pkg/shims/storagetransfer.TestListTransferJobsPagination` | No |
...
| `workflows.googleapis.com` | experimental | file | experimental | — (not promoted) | Yes | workflow metadata and bounded local execution<br>Package tests only: `pkg/shims/workflows.TestCreateWorkflow`<br>`pkg/shims/workflows.TestListWorkflows`<br>`pkg/shims/workflows.TestDeleteWorkflow`<br>`pkg/shims/workflows.TestPersistAndReload`<br>`pkg/shims/workflows.TestRestartMarksActiveExecutionFailed` | No |
