...
| Compute Engine (`compute.googleapis.com`) | Instances, networks, subnetworks, firewall rules, load-balancer metadata, and the stable subnetwork counter in profile state; security policies and LROs remain in memory; Docker containers and bridges may survive | Rehydrated VMs remain `METADATA_ONLY`, clear stale host-port mappings, and are never recreated implicitly. A supported custom VPC reconciles only its exact project/profile-owned bridge identity and labels; the immutable Docker ID, bridge driver, and single CIDR/IPAM must match, otherwise reconciliation refuses adoption. Explicit-URL load-balancer backends continue to proxy after restart; Docker-backed instance endpoints require reconciliation. Docker bridge state is runtime data excluded from export/import. |
...
| Memorystore for Memcached (`memcache.googleapis.com`) | Memcached metadata is profile-persisted, including instances and admitted create/update/delete operation associations; nodes use exact-owned Memcached containers | Cache contents are intentionally ephemeral and excluded from metadata export. Startup reconciles only exact profile/service/resource ownership labels and observable backend state. It does not replay create/update side effects after interruption; unresolved ownership or inspection failures retain the same association for later reconciliation, while delete is retried only for an exact-owned backend. |
