...
| Memorystore for Memcached (`memcache.googleapis.com`) | Memcached metadata is profile-persisted; instances use exact-owned Memcached containers | Cache contents are intentionally ephemeral and excluded from metadata export. Startup reconciles only exact profile/service/resource ownership labels. |
